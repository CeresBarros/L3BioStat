...
\shadowbox{\parbox{15cm}{\textbf{Objectifs de la séance} \\ \`A partir des jeux de données proposés et de la problématique biologique soulevée, il s'agit de choisir le bon test statistique et de s'initier avec le logiciel \R}}
...
Chacune des variables constituant le \dataframe\ peut être obtenue individuellement. Il existe plusieurs façon pour réaliser cela. La plus simple est certainement de faire suivre le nom de l'objet contenant le \dataframe\ du signe $\$$ et du nom de la variable souhaité : $peuplier\$Diametre$
...
La selection combinée de certaines variables pour certains individus s'exprime donc selon le modèle suivant :
...
Lorsque l'on calcule un intervalle de confiance, c'est la plupart du temps sous l'hypothèse d'une loi normale. Pour calculer notre intervalle de confiance en connaissance de cause, nous devons donc en premier tester la normalité
des données.
...
Vous pouvez relancez plusieurs fois ces commandes de manière à observer la fluctuation de l'aspect du <<qqplot>>.
...
\item un nombre de degrés de liberté en second argument
\end{itemize}
